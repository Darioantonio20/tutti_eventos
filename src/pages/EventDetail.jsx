...
const EventDetail = () => {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans text-slate-900 pb-20">
                ...
                <div className="lg:col-span-8 space-y-10">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="space-y-4 max-w-sm">
                                    ...
                                    <div className="w-full h-3 bg-slate-50 rounded-full overflow-hidden">
                                        <motion.div initial={{ width: 0 }} animate={{ width: '85%' }} transition={{ duration: 1.5, ease: "easeOut" }} className="h-full bg-[#2eb8ff] rounded-full shadow-[0_2px_10px_rgba(46,184,255,0.3)]" />
                                    </div>
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <button className="flex items-center gap-2 text-slate-400 hover:text-slate-600 transition-colors font-bold text-xs uppercase tracking-widest">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="flex gap-5">
                                        ...
                                        <div className="flex-1 min-w-0 py-1 flex flex-col">
                                            ...
                                            <div className="flex items-center justify-between mt-auto">
                                                ...
                                                <button
                                                    onClick={() => service.status === 'completed' ? (setSelectedService(service), setActiveModal('service')) : showToast('Buscando proveedores...')}
                                                    className={`
                            ...
                            <button onClick={() => setActiveModal('add-service')} className="border-2 border-dashed border-slate-200 rounded-[2.2rem] p-6 flex flex-col items-center justify-center gap-4 group hover:border-[#2eb8ff]/40 transition-all hover:bg-white min-h-[140px] bg-slate-50/30">
                ...
                <div className="lg:col-span-4 space-y-10">
                    <Card className="p-0 border-none bg-gradient-to-br from-[#2eb8ff] to-[#00d2ff] shadow-[0_30px_60px_-12px_rgba(46,184,255,0.45)] overflow-hidden text-white relative group">
                        <div className="p-9 space-y-9 relative z-10">
                            ...
                            <button onClick={() => navigate('/mensajes')} className="w-full py-5 bg-black/10 hover:bg-black/20 transition-all rounded-[1.8rem] flex items-center justify-center gap-3 backdrop-blur-sm group/btn border border-white/5">
                    ...
                    <Card className="p-9 border-slate-50 space-y-9 bg-white shadow-sm">
                        <h3 className="text-xl font-black text-slate-800 tracking-tight mb-2">Acciones Rápidas</h3>
                        <div className="space-y-4">
                            {[
                                ...
                                <button key={action.id} onClick={() => action.id === 'payment' ? setActiveModal('payment') : action.id === 'share' ? showToast('¡Enlace copiado al portapapeles!') : showToast('Ajustes en desarrollo')} className="w-full flex items-center gap-5 p-3 rounded-[1.4rem] hover:bg-slate-50 transition-all group/action">
            ...
            <Modal isOpen={activeModal === 'service'} onClose={() => setActiveModal(null)} title={selectedService?.name}>
                <div className="space-y-6">
                    <div className="bg-slate-50 p-6 rounded-3xl">
                        <p className="text-slate-500 font-bold italic text-sm leading-relaxed">{selectedService?.details}</p>
                    </div>
            ...
            <Modal isOpen={activeModal === 'add-service'} onClose={() => setActiveModal(null)} title="Añadir Nuevo Servicio">
                <div className="space-y-4">
                    <input className="w-full p-4 bg-slate-50 rounded-2xl border-none font-bold text-sm" placeholder="Nombre del servicio..." />
                    ...
                    <button onClick={() => { setActiveModal(null); showToast('¡Servicio añadido!'); }} className="w-full py-4 bg-[#2eb8ff] text-white rounded-2xl font-black text-sm shadow-lg shadow-blue-500/20 active:scale-95 transition-transform">Confirmar</button>
                </div>
            </Modal>

            <Modal isOpen={activeModal === 'payment'} onClose={() => setActiveModal(null)} title="Registrar Pago">
                <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <span className="text-[10px] font-black uppercase text-slate-400 block mb-1">Pagado</span>
                    ...
                    <button onClick={() => { setActiveModal(null); showToast('¡Pago registrado!'); }} className="w-full py-4 bg-green-500 text-white rounded-2xl font-black text-sm shadow-lg shadow-green-500/20">Realizar Pago</button>
                </div>
            </Modal>
        </div>
    );
